    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                @php
                    $recipeMap = $product->items->mapWithKeys(fn ($item) => [$item->id => $item->pivot->quantity_required])->all();
                @endphp
                <form action="{{ route('products.update', $product) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" class="mt-1 block w-full" required>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Description</label>
                        <textarea name="description" class="mt-1 block w-full">{{ old('description', $product->description) }}</textarea>
                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-100">
                                    @forelse($items as $item)
                                        @php
                                            $oldQty = old('recipe.' . $item->id, $recipeMap[$item->id] ?? 0);
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('products.index') }}" class="me-2">Cancel</a>
